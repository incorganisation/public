...
no_of_browsers = 6000
no_of_elements = 2
url = 'https://increvenue98.gotrackier.com/sl/o9phvuT/?pub_id=22&source='

i = 0
count = 0

while(True):  #this can be done infinetly..using while true
    try:
        usr_agnt_mobile=[
...
"Mozilla/5.0 (iPhone; CPU iPhone OS 5_1_1 like Mac OS X) AppleWebKit/534.46 (KHTML, like Gecko) Version/5.1 Mobile/9B206 Safari/7534.48.3"
    ...
    # *********OPENING BROWSERS WITH URL***********
        if(i<no_of_browsers):
            opts = Options()
            opts.add_argument("user-agent="+ usr_agnt_mobile[randint(0,len(usr_agnt_mobile)-1)])
            opts.add_argument('--no-sandbox');
            open_browser = webdriver.Chrome(executable_path="/root/chromedriver",chrome_options=opts)
            time.sleep(1)
            open_browser.set_window_size(360,640);
            open_browser.get(url)
            time.sleep(3);
            opening_time = datetime.datetime.now()
            list_browser.append((open_browser, opening_time))
            i +=1
      #      open_browser.get_screenshot_as_file('initial'+str(i)+'.png')
    # *********CHECK TIME AND CLOSE**********
        if ((datetime.datetime.now() - list_browser[0][1]).seconds >= time_of_session + randint(5,10)):
            count += 1
            list_browser[0][0].get_screenshot_as_file('final'+str(i)+'.png')
            list_browser[0][0].quit()
            list_browser.pop(0)
    except:
        continue 
